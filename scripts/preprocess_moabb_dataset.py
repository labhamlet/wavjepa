...
def process(dataset_name, subject_id, save_dir, preprocessors, offset, dataset_kwargs):
    dataset = MOABBDataset(dataset_name, subject_ids=[subject_id], dataset_kwargs=dataset_kwargs)
...
def main(dataset_name, path, dataset_kwargs: dict = None, sfreq=128, l_freq=0.5, h_freq=40,
         suffix=''):
...
def main_main(config_path: str):
    ...
    for dataset_config in config['datasets']:
        ...
        main(**{f'dataset_{k}': v for k, v in dataset_config.items()},
             **kwargs)
