...
class JEPA(pl.LightningModule):
    ...
    def _get_pos_embed_params(self, embedding_dim):
        ...
        # TODO! Remove this total patches later.
            ...
            not self.is_spectrogram
            and self.in_channels == 2
            and (self.total_patches == 400)
        ):
            ...
            pos_embed_data = get_binaural_pos_embed(
                embedding_dim, time_steps=self.total_patches // self.in_channels
            )
        elif (
            not self.is_spectrogram
            and self.in_channels == 2
            and (self.total_patches == 200)
        ):
            ...
            raise Exception(
                f"Not implemented for more in_channels, {self.in_channels}, {self.total_patches}"
            )
    ...
    def prepare_batch(
        ...
        if self.sr != ORIGINAL_SR:
            generated_scene = self.resample(
                generated_scene, resample_sr=self.sr, original_sr=ORIGINAL_SR
            )
        assert generated_scene.shape[1] <= self.in_channels, (
            f"Generated scene has more channels than in channels, {generated_scene.shape}, {self.in_channels}"
        )
        ...
        if generated_scene.shape[1] < self.in_channels:
            generated_scene = repeat(
                generated_scene,
                "B C L_full -> B (repeat C) L_full",
        ...
        indices = rand_starts.unsqueeze(-1) + torch.arange(
            self.target_length, device=self.device
        )

        # Expand scene and indices for gathering
        # scene: (B, C, L_full) -> (B, 1, C, L_full) -> (B, nr_samples, C, L_full)
        scene_expanded = generated_scene.unsqueeze(1).expand(
            -1, self.nr_samples_per_audio, -1, -1
        )
        ...
        normalized_audios = (return_audios - mean) / (
            std + 1e-5
        ...
        return (
            flattened[idx, ...],
            self.collate_fn(ctx_masks),
            self.collate_fn(target_indices),
            self.collate_fn(ctx_and_target_masks),
        )

    def training_step(self, batch: torch.Tensor, batch_idx: int) -> ForwardReturn:
        audio_input, ctx_masks, target_indices, ctx_and_target_masks = (
        ...
        with torch.amp.autocast(
            "cuda", enabled=False
        ):  # Force FP32 computation for stability
            self._step_teacher()
    ...
    def forward(
        self, audio: torch.Tensor, ctx_masks, target_indices, ctx_and_target_masks
    ) -> ForwardReturn:
        """
        Args:
            batch: torch.Tensor
        ...
        contextual_features = self.encoder_forward(
            local_features, src_key_padding_mask=ctx_masks
        )
    ...
    def encoder_forward(
        ...
        if self.use_gradient_checkpointing and self.training:
            ...
            contextual_features = self.encoder(
                x_contexts, src_key_padding_mask=src_key_padding_mask
            )
    ...
    def get_audio_representation(self, audio: torch.Tensor, padding_mask: torch.tensor):
        ...
        contextual_features = self.encoder_forward(
            local_features, src_key_padding_mask=padding_mask
        )
        return contextual_features
